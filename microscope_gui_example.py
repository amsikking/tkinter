...
class GuiAcquisition:
    def __init__(self, master):
        ...
        self.focus_piezo_z_um = 0 # -> code removed.
        self.XY_stage_position_mm = [0, 0] # -> code removed.
        # get scope ready:
        self.loop_snoutfocus()
        print('calling: self.scope.acquire()')
    ...
    def loop_snoutfocus(self):
        if not self.running_aquisition.get():
            print('calling: self.scope.snoutfocus()')
        self.frame.after(60000, self.loop_snoutfocus)
...
# -> comment out to avoid continuos print in live mode
##            print('calling: self.scope.acquire()')
    ...
    def init_snap_and_save_button(self):
        snap_and_save_button = tk.Button(
            self.frame, text="Snap volume and save",
    ...
    def snap_volume_and_save(self):
        ...
        print("calling: self.scope.acquire(filename='snap.tif', " +
              "folder_name=folder_name, description=self.description.text")
    ...
    def init_description_textbox(self):
        self.description = tki_cw.Textbox(
            self.frame,
            label='Description',
    ...
    def init_run_aquisition_button(self):
        ...
        run_aquisition_button = tk.Button(
            self.frame, text="Run aquisition",
    ...
    def run_acquisition(self):
        ...
        for i in range(self.acquisitions.spinbox_value):
            if i == 0: # avoid first delay_s
                print("calling: self.scope.acquire(filename='%06i.tif'%i, " +
        ...
        print("calling: self.scope.finish_all_tasks()")
    ...
    def apply_settings(self, single_volume=False, _print=False):
        ...
        if power_per_channel != self.power_per_channel:
            print('calling: self.scope.apply_settings(' +
        ...
        if height_px != self.height_px:
            print('calling: self.scope.apply_settings(height_px=height_px)')
            self.height_px = height_px
        if width_px != self.width_px:
            print('calling: self.scope.apply_settings(width_px=width_px)')
        ...
        if single_volume:
# -> comment out to avoid continuos print in live mode
##            print('calling: self.scope.apply_settings(volumes_per_buffer=1)')
            pass
        else:
            print('calling: self.scope.apply_settings(' +
...
##        print('calling: self.scope.finish_all_tasks()')
    ...
    def close(self):
        print('calling: self.scope.close()')
